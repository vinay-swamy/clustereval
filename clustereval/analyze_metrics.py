...
def strategy_1_recluster(all_permute_metrics, all_labels, stability_threshold=.8, purity_threshold=.8,max_sets_to_match=3,min_pass_cluster_delta = 2, min_knn_delta  = 20, max_results_to_eval=3 ):
    all_permute_metrics = all_permute_metrics.assign(nCluPass_stability = lambda x: x['permute_stability'] > stability_threshold, 
                                                     nCluPass_purity = lambda x: x['permute_purity'] > purity_threshold ).assign(nCluPass_both = lambda x: x['nCluPass_stability'] & x['nCluPass_purity'] )
    ...
    for mapping in match_results[:max_results_to_eval]:
        ...
        results_to_recluster.append((query_knn, target_knn, mapping_result, res ))
    return results_to_reclusterk